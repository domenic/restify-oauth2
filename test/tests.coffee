...
beforeEach ->
    ...
    @requiresTokenPredicate = sinon.stub()
    @requiresTokenPredicate.returns(true)
    @requiresTokenPredicate.withArgs(sinon.match.has("path", "/public")).returns(false)
...
describe "For requests to resources that do not require a token", ->
