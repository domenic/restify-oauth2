...
module.exports = function grantToken(req, res, next, options) {
    ...
    options.hooks.validateClient(clientId, clientSecret, scope, function (error, result) {
        if (error) {
            return next(error);
        }
        ...
        options.hooks.grantUserToken(username, password, scope, function (error, token) {
            if (error) {
                return next(error);
            }
