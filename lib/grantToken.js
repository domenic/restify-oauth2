...
module.exports = function grantToken(req, res, next, options) {
    function sendOAuthError(errorClass, errorType, errorDescription) {
        ...
        next(new restify[errorClass + "Error"](errorDescription, body));
    }
    ...
    if (req.body.grant_type !== "password") {
        return sendBadRequestError("unsupported_grant_type", "Only grant_type=password is currently supported.");
    }
    ...
    options.validateClient(clientId, clientSecret, function (error, result) {
        if (error) {
            return next(error);
        }

        if (!result) {
            sendUnauthorizedError("Client ID and secret did not validate.");
        }

        var username = req.body.username;
        var password = req.body.password;

        options.grantToken(username, password, function (error, token) {
            if (error) {
                return next (error);
            }

            if (!token) {
                sendUnauthorizedError("Username and password did not authenticate.");
            }
