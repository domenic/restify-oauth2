...
module.exports = function finishGrantingToken(allCredentials, options, req, res, next) {
    ...
    options.hooks.grantScopes(allCredentials, scopesRequested, req, function (error, scopesGranted) {
        ...
        var responseBody = {
            access_token: allCredentials.token,
            token_type: "Bearer",
            expires_in: options.tokenExpirationTime
        };

        if (_.has(allCredentials, "refresh_token")) {
            responseBody.refresh_token = allCredentials.refreshToken;
            responseBody.refresh_expires_in = options.refreshExpirationTime;
        }
