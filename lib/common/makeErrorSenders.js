...
module.exports = function makeErrorSenders(grantTypes) {
    ...
    function sendAuthenticationRequired(res, options, error) {
        setLinkHeader(res, options);
        setWwwAuthenticateHeaderWithoutErrorInfo(res, options);
        res.send(error);
    }

    function sendInsufficientAuthorization(res, options, error) {
        setLinkHeader(res, options);
        res.send(error);
    }
    ...
    return {
        ...
        authenticationRequired: function (res, options, message) {
            if (message === undefined) {
                message = "Authentication via bearer token required. Follow the oauth2-token link to get one!";
            }

            sendAuthenticationRequired(res, options, new restify.UnauthorizedError(message));
        },

        insufficientAuthorization: function (res, options, message) {
            if (message === undefined) {
                message = "Insufficient scope";
            }

            sendInsufficientAuthorization(res, options, new restify.Forbidden(message));
        },
