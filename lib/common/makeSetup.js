...
module.exports = function makeSetup(grantTypes, reqPropertyName, requiredHooks, grantToken) {
    ...
    var handleAuthorizeResource     = require("./makeHandleAuthorizeResource")(errorSenders);
    ...
    return function restifyOAuth2Setup(server, options) {
        ...
        options = _.defaults(options, {
            ...
            reqPropertyName: reqPropertyName
        });
        ...
        server.use(function ccOAuth2Plugin(req, res, next) {
            ...
            if (req.method === "POST" && req.path() === options.tokenEndpoint) {
                ...
                req[options.reqPropertyName] = null;
                next();
            }
        });

        if (typeof(options.hooks.authorizeToken) === 'function') {
            server.use(function ccOAuth2AuthorizePlugin(req, res, next) {
                handleAuthorizeResource(req, res, next, options);
            });
        }
    };
};
