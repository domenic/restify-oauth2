...
module.exports = function makeSetup(grantTypes, reqPropertyName, requiredHooks, grantToken) {
    ...
    var skip = function (url, options) {
        console.log('inside skip: ', url);
        console.dir(options);
        if (options.exclude) {
            for (var i=0; i<options.exclude.length; ++i) {
                if (options.exclude[i] === url)
                    return true;
            }
        }
        return false;
    ...
    return function restifyOAuth2Setup(server, options) {
        ...
        server.post(options.tokenEndpoint, options.plugins, function (req, res, next) {
