...
module.exports = function makeSetup(grantTypes, requiredHooks, grantToken) {
    ...
    return function restifyOAuth2Setup(server, options) {
        ...
        // TOBY
        //
        // Add jsonBodyParser middleware here as we don't set it globally for all routes since
        // doing so prevents streaming files to the server, as needed by /command/resource/upload.
        // (see nqm-command-validators/lib/file-upload-handler)
        // 
        // TOBY
        server.post(options.tokenEndpoint, restify.jsonBodyParser(), function (req, res, next) {
